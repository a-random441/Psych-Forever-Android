...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		if(FileSystem.exists(SUtil.getStorageDirectory() + modsListPath))
		{
			...
			for (i in 0...stuff.length)
			{
				...
				else // Sort mod loading order based on modsList.txt file
				{
					...
					if (sys.FileSystem.isDirectory(SUtil.getStorageDirectory() + path) && !Paths.ignoreModFolders.contains(splitName[0]) && !disabledMods.contains(splitName[0]) && !directories.contains(path + '/'))
					{
		...
		for (i in 0...directories.length) {
			...
			if(FileSystem.exists(SUtil.getStorageDirectory() + directory)) {
				...
				for (file in FileSystem.readDirectory(SUtil.getStorageDirectory() + directory))
				{
					...
					if (!sys.FileSystem.isDirectory(SUtil.getStorageDirectory() + path) && file.endsWith('.json'))
					{
